...
% dynamic formation maneuvering.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear
clc
close all
load 'SI_dynamic_fomation_manv_results'
lw = 2;                                      % line width
ms = 12;                                     % MakerSize
pausetime = 0.1;                             % Pause time
gifName = 'SI_dynamic_fomation_manv.gif';    % File name for saved gif
iniName = 'SI_dynamic_fomation_manv_ini.gif';% File name for snapshot 
...
set(gca,'DataAspectRatio',[1 1 1],'Box','on','FontSize',18,...
    'Position',[.07 .07 .9 .9])
xlabel('x')
ylabel('y')
for ii=1:1:length(t)        % Go through time from 0 to tfinal
    ...
    if ii == 1
        ...
        legend(hh1,'Intitial position','Location','SouthWest')
        f = getframe(gcf);
        f = frame2im(f);
