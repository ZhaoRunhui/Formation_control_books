...
set(gca,'XLim',[-1 1.5],'YLim',[-1 1],'Box','on',...
    'DataAspectRatio',[1 1 1],'FontSize',fs,'YTick',[-1 -0.5 0 0.5 1])
xlabel('x')
ylabel('y')
for ii=1:5:length(t)        % Go through time from 0 to tfinal
    ...
    elseif ii == length(t) % At time tfinal, draw final configuration
        ...
        % in time between 0 and tfinal, make gif writemode be append
        f = getframe(gcf);
        f = frame2im(f);
        [im,map] = rgb2ind(f,128);
        imwrite(im,map,gifName,'gif','WriteMode','append',...
